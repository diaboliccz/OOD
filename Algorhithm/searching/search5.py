...
product = [int(i) for i in inp[0].split()]
box = int(inp[1])
def findMinWeight(product, box):
    left = max(product)
    right = sum(product)
    while left < right:
        mid = (left + right) // 2
        ...
        for i in range(len(product)):
            if sum_weight + product[i] > mid:
                count += 1
                sum_weight = product[i]
            else:
                sum_weight += product[i]
        if count <= box:
            right = mid
            ...
            left = mid + 1
        print(left, right, count, sum_weight)
    return left

print("Minimum weigth for {0} box(es) = {1}".format(box, findMinWeight(product, box)))